...
────────────────────────────────────────────────────────────────────────
DECISION REASON (new): explaining WHY the go/no-go decision was made
────────────────────────────────────────────────────────────────────────
Previously the only explanation shown for a GO / NO-GO / ESCALATE decision
was the AI-written "summary" field, which was often a vague one-liner (e.g.
"We should bid with escalation items") that didn't say WHICH checklist
items actually drove the outcome. It could also go stale, since the
verification pass can correct individual checklist items AFTER the AI
wrote that summary.

_build_decision_reason() fixes this by building a clear, specific "why"
explanation DETERMINISTICALLY from the final checklist itself (no extra AI
call, so it's free and instant), listing out exactly which items failed
(for NO-GO), which items need review (for ESCALATE), or confirming a clean
pass (for GO). It's called from inside _recompute_go_no_go_scores(), which
already runs both after the initial extraction AND after verification
corrections are applied -- so the reason is guaranteed to always match the
final, corrected checklist and never goes stale.
"""
...
def _build_decision_reason(checklist: List[Dict[str, Any]], decision: str) -> str:
    """
    Builds a clear, specific explanation for why the overall decision is
    GO, NO-GO, or ESCALATE, based on which checklist items actually drove
    that outcome. Computed deterministically from the checklist itself
    (not a separate AI call) so it automatically stays accurate even after
    the verification pass corrects individual item scores/statuses.
    """
    if not checklist:
        return "No checklist items were available to evaluate, so a decision could not be justified."

    nogo_items = [i for i in checklist if i.get('status') == 'NO-GO']
    escalate_items = [i for i in checklist if i.get('status') not in ('GO', 'NO-GO')]

    def _list_items(items):
        return "; ".join(
            f"{i.get('item', 'Unknown')} ({i.get('reason', 'no reason given')})" for i in items
        )

    if decision == 'NO-GO':
        return (
            f"Recommended NO-GO because {len(nogo_items)} checklist item(s) failed outright: "
            f"{_list_items(nogo_items)}. These are hard blockers under our company checklist, "
            f"so bidding is not recommended unless these specific issues can be resolved."
        )
    elif decision == 'GO':
        base = (
            f"Recommended GO because all {len(checklist)} checklist items met our requirements "
            f"(scored 7 or higher), with no outright failures"
        )
        if escalate_items:
            base += f", though {len(escalate_items)} item(s) could use a quick review: {_list_items(escalate_items)}."
        else:
            base += "."
        return base
    else:  # ESCALATE / conditional
        reason = (
            f"Recommended ESCALATE (proceed with caution) because {len(escalate_items)} checklist "
            f"item(s) are unclear or need management review before bidding: {_list_items(escalate_items)}."
        )
        if nogo_items:
            reason += f" Additionally, {len(nogo_items)} item(s) failed outright: {_list_items(nogo_items)}."
        return reason
...
def _recompute_go_no_go_scores(result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # NEW: build a clear, item-level "why" explanation for the decision.
    # Runs every time this function runs -- both right after the initial
    # extraction AND after the verification pass applies corrections -- so
    # it always reflects the FINAL checklist, never a stale one.
...
class RiskAgent(BaseAgent):
    ...
    def fallback(self, error: str) -> Dict[str, Any]:
        return {
            ...
            "decision_reason": (
                "A decision reason could not be generated because the risk analysis "
                f"encountered an error: {error}. Please review the RFP manually."
            ),
            "conflicts": [],
            "conflict_summary": (
...
def run_agents_parallel(models, text: str) -> Dict[str, Any]:
    ...
    if checklist:
        ...
        # decision_reason) to match the FINAL, corrected checklist.
